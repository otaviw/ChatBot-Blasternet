...
function SupportTicketChatTab({ ticketId, viewerRole = 'company' }) {
  ...
  const requestMessages = useCallback(() => {
    return viewerRole === 'admin'
      ? supportTicketChatService.listForAdmin(ticketId)
      : supportTicketChatService.listForCompany(ticketId);
  }, [ticketId, viewerRole]);
  ...
  const fetchMessages = useCallback(
    async ({ silent = false } = {}) => {
      ...
      if (silent) {
        ...
        try {
          ...
          setMessagesError(err?.response?.data?.message || 'Falha ao carregar mensagens do chat.');
        } finally {
          setRefreshing(false);
        }
        return;
      ...
      const { data: response } = await runMessagesRequest(
        () => requestMessages(),
        { mapError: (err) => err?.response?.data?.message || 'Falha ao carregar mensagens do chat.' }
      );
    ...
    [clearMessagesError, requestMessages, runMessagesRequest, setMessagesError, ticketId]
  );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessagesError(err?.response?.data?.message || 'Falha ao enviar mensagem.');
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <section className="border border-[#e3e3e0] rounded-lg p-4">
      ...
      {messagesError && (
        <p className="text-sm text-red-600 mb-3">{messagesError}</p>
      )}

      <AsyncState loading={loading} loadingSlot={<p className="text-sm text-[#64748b]">Carregando mensagens...</p>}>
