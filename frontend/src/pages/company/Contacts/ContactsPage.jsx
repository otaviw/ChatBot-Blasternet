...
function ContactsPage() {
  ...
  const [newDefaultSenderNumberId, setNewDefaultSenderNumberId] = useState('');
  const [newDefaultAttendantId, setNewDefaultAttendantId] = useState('');
  const [newSkipBot, setNewSkipBot] = useState(false);
  const [modalError, setModalError] = useState('');
  const [activeSenderNumbers, setActiveSenderNumbers] = useState([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!canceled) setActiveSenderNumbers(getActiveCompanyNumbers(response?.data?.company ?? response?.data ?? null));
      } catch (_error) {
        if (!canceled) setActiveSenderNumbers([]);
      }
    })();
    return () => {
      canceled = true;
  ...
  const handleCreateContact = async (event) => {
    ...
    try {
      await createContact({
        name: newName,
        phone: newPhone,
        default_sender_number_id: newDefaultSenderNumberId || null,
    ...
    <Layout role="company" companyName={data?.user?.company_name} onLogout={logout}>
      <section className="contacts-page">
        <header>
          ...
          <p className="app-page-subtitle">Gerencie sua base e numero padrao de envio por contato.</p>
        </header>
        ...
        <div className="app-panel contacts-toolbar">
          ...
          <div className="contacts-toolbar-actions">
            <input ref={csvInputRef} type="file" accept=".csv,text/csv,text/plain" className="contacts-hidden-input" onChange={handleCsvSelected} />
      ...
      <ContactDetailModal
        ...
        activeSenderNumbers={activeSenderNumbers}
      />
        ...
        <div className="contacts-modal-overlay" onClick={() => !creating && setIsModalOpen(false)} role="presentation">
          <div className="contacts-modal app-panel" role="dialog" aria-modal="true" aria-label="Novo contato" onClick={(event) => event.stopPropagation()}>
            <div className="contacts-modal-header">
              ...
              <button type="button" className="contacts-close-btn" onClick={() => !creating && setIsModalOpen(false)} disabled={creating} aria-label="Fechar modal">x</button>
            </div>
            <form onSubmit={handleCreateContact} className="contacts-form">
              <label htmlFor="contacts-new-name" className="contacts-label">Nome</label>
              <input id="contacts-new-name" type="text" className="app-input" value={newName} onChange={(event) => setNewName(event.target.value)} disabled={creating} required />
              <label htmlFor="contacts-new-phone" className="contacts-label">Telefone</label>
              <input id="contacts-new-phone" type="tel" className="app-input" value={newPhone} onChange={(event) => setNewPhone(event.target.value)} disabled={creating} required />
              <label htmlFor="contacts-new-default-sender" className="contacts-label">Numero padrao de envio</label>
              <select id="contacts-new-default-sender" className="app-input" value={newDefaultSenderNumberId} onChange={(event) => setNewDefaultSenderNumberId(event.target.value)} disabled={creating || activeSenderNumbers.length === 0}>
                <option value="">Sem numero padrao</option>
                {activeSenderNumbers.map((number) => <option key={String(number.id)} value={String(number.id)}>{String(number.label || number.id)}</option>)}
              </select>
              <label htmlFor="contacts-new-default-attendant" className="contacts-label">Atendente padrao</label>
              <select id="contacts-new-default-attendant" className="app-input" value={newDefaultAttendantId} onChange={(event) => { const v = event.target.value; setNewDefaultAttendantId(v); if (!v) setNewSkipBot(false); }} disabled={creating}>
              ...
              <label className="contacts-checkbox-label" htmlFor="contacts-new-skip-bot">
                <input id="contacts-new-skip-bot" type="checkbox" checked={newSkipBot} disabled={creating || !newDefaultAttendantId} onChange={(event) => setNewSkipBot(event.target.checked)} />
                <span>Pular bot e ir direto para atendente</span>
              </label>
              {modalError ? <p className="text-xs text-red-600">{modalError}</p> : null}
              <div className="contacts-modal-actions">
                <button type="button" className="app-btn-secondary" onClick={() => !creating && setIsModalOpen(false)} disabled={creating}>Cancelar</button>
