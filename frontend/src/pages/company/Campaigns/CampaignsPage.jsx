...
function CampaignsPage() {
  ...
  const loadCampaigns = useCallback(async ({ silent = false } = {}) => {
    ...
    const { data: response, error: loadError } = await runCampaignsRequest(
      ...
      { mapError: (err) => err?.response?.data?.message ?? 'N�o foi poss�vel carregar campanhas.' }
    );
  ...
  const openCreateModal = () => {
    ...
    void fetchAllCompanyContacts()
      ...
      .catch((err) => {
        setContacts([]);
        setContactsError(err?.response?.data?.message ?? 'N�o foi poss�vel carregar contatos.');
      })
      .finally(() => {
  ...
  const handleImportContactsCsv = async (file) => {
    ...
    try {
      ...
      showSuccess(`Importa��o conclu�da: ${imported} importados, ${skipped} ignorados.`);
    } catch (err) {
      const message = err?.response?.data?.message ?? 'N�o foi poss�vel importar CSV de contatos.';
  ...
  const handleCreateCampaign = async (formData) => {
    ...
    try {
      ...
      const message =
        ...
        'N�o foi poss�vel criar campanha.';
  ...
  const handleStartCampaign = async (campaignId) => {
    ...
    try {
      ...
      showError(err?.response?.data?.message ?? 'N�o foi poss�vel iniciar o envio.');
    } finally {
      setStartingId(null);
    }
  };
    ...
    return (
      <Layout role="company" onLogout={logout}>
        <section className="campaigns-page">
          <h1 className="app-page-title">Campanhas</h1>
          <div className="app-panel">
            <ErrorMessage message="N�o foi poss�vel carregar a p�gina." />
          </div>
        </section>
      </Layout>
    );
